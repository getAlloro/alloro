...
- The single most important recommendation (the \`top_recommendations\` entry) must reach the "how did they know that?" bar, the same discipline as the Checkup's public-data cards: (1) name a specific competitor by name or a specific profile factor, never "your competitor"; (2) cite one real number or pattern from the input that the owner cannot discover by looking at their own Google profile, ideally by cross-referencing two data points into one private-feeling insight; (3) end in one clear action the owner can do THIS WEEK (a verb and an object). NEVER ship these commodity findings, which are obvious and not surprising: "you have fewer reviews than [competitor]", "your rating is lower than average", "you should get more reviews", "post more", or "add photos".
- Leader coherence: on any dimension where the practice already leads or sits in the top set (rank, rating, or any factor where it is ahead), never tell the owner to "catch up", "close the gap", or "improve" on that dimension. Affirm the lead and choose a different, winnable move. The competitor-review-count rule below is one instance of this.
- When a competitor's total review count is many multiples of the practice's AND that competitor's \`primary_category\` is a generalist category (for example "Dentist" or "Dental clinic") while the client is a specialist, do NOT frame the review-count difference as a gap the practice must close. Reframe it honestly: the practice is not losing to a better practice, it is being outshown by a louder, broader one. Choose a winnable move (a comparison against a like-for-like specialist competitor, or a review recency and velocity signal), never "match their review count".
...
- For \`overview_card.text\`, choose the format by the sampled Google Maps position in \`search_position.position\` (not by any other rank field), using these four bands:
  1. Sampled position #1: "[Client or location name] holds a dominant #1 Local Search Ranking with a X Alloro Health Score. Recommended Action: [one direct action]." Use "protect the lead" only in this band.
  2. Sampled position #2 or #3 (the top set): affirm first, never frame it as behind. "[Client or location name] is in the top three when patients search near [city], with a X Alloro Health Score. Recommended Action: [one action that protects or widens the top-set standing]." Do NOT tell a top-set practice to "close the gap to #1", "reach #1", or "improve the position"; being in the top three is the win.
  3. Sampled position outside the top set (position greater than 3, or \`search_position.not_in_top_20\` is true): "[Client or location name] is currently #N in Local Search with a X Alloro Health Score. Recommended Action: [one action toward breaking into the top 20]." Do NOT tell an outside-the-top-20 practice to "break into the top three"; that overshoots the goal the code already sets.
  4. Sampled position unknown (\`search_position.position\` is null, or \`search_position.status\` is \`bias_unavailable\` or \`api_error\`): "[Client or location name] has a X Alloro Health Score; local position is pending this month. Recommended Action: [one direct action]." Never fall back to #1 or a made-up rank.
- In \`overview_card.text\`, use the practice or location name, the sampled local position, and the rounded owner-visible score. Use "Alloro Health Score" in this field instead of "Local Search Score".
...

{
  ...
  "overview_card": {
    "text": "<owner-readable main card copy following the four sampled-position bands defined in the Rules: #1 = dominant lead; #2 or #3 = affirmed as in the top three, never told to chase #1; outside the top 20 = break into the top 20; unknown position = 'local position is pending this month', never a fabricated #1. Include review reply context only when engagement_summary says unanswered reviews exist.>",
