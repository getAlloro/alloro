...
- For \`overview_card.text\` when the sampled rank is NOT #1, choose the band from the real \`search_position.position\`:
  - Position 2 or 3 (top set): affirm first, never frame as behind. "[Client or location name] is in the top three when patients search for [specialty] near [city], with a X Alloro Health Score. Recommended Action: [one action that protects or widens the top-set standing]." Never tell a top-set practice to close the gap to #1.
  - Position greater than 3, or \`search_position.not_in_top_20\` is true: the goal is breaking into the top 20 (per the Search Position rule above), not the top three. "[Client or location name] is currently #N in Local Search with a X Alloro Health Score. Recommended Action: [one action toward breaking into the top 20]."
  - Position unknown (\`search_position.position\` is null, or \`status\` is \`bias_unavailable\` or \`api_error\`): say "position pending this month" and never fall back to #1 or a fabricated rank.
...
- The \`overview_card.text\` recommended action should point to the clearest owner action. Use "protect the lead" only when the sampled rank is #1. For a top-set (2 or 3) practice, use language that protects or widens the standing, never "close the gap to #1". For a practice outside the top set, aim at "break into the top 20". Never frame a top-set practice as deficient.
...

{
  ...
  "overview_card": {
    "text": "<owner-readable main card copy. If rank is #1, use: '[Client or location name] holds a dominant #1 Local Search Ranking with a X Alloro Health Score. Recommended Action: [one direct action].' If rank is not #1, follow the top-set / outside-top-set / position-pending bands in the Rules section above, and never frame a top-set practice as behind. Include review reply context only when engagement_summary says unanswered reviews exist.>",
