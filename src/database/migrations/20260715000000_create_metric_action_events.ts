...
export async function up(knex: Knex): Promise<void> {
  await knex.schema.createTable(TABLE, (table) => {
    ...
    table.unique(
      ["action_type", "source_type", "source_id"],
      { indexName: "uq_metric_action_events_source" }
    );
    table.index(
      [
        "organization_id",
    ...
    table.check(
      "affected_count > 0",
      [],
      "metric_action_events_affected_count_check"
    );
    table.check(
      "active_until > occurred_at",
      [],
      "metric_action_events_active_window_check"
    );
  });
}
