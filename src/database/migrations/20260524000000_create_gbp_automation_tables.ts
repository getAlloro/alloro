...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.raw(`
    ...
    ON ${SETTINGS_TABLE}(organization_id)
    WHERE location_id IS NULL
  `);
  await knex.raw(`
    CREATE UNIQUE INDEX gbp_automation_settings_org_location_unique
    ON ${SETTINGS_TABLE}(organization_id, location_id)
  ...
  await knex.schema.createTable(WORK_ITEMS_TABLE, (table) => {
    ...
    table
      ...
      .onDelete("SET NULL");
    table
      .integer("published_by_user_id")
      .references("id")
      .inTable("users")
      .onDelete("SET NULL");
    table
      .integer("rejected_by_user_id")
      .references("id")
      .inTable("users")
      .onDelete("SET NULL");
  ...
  await knex.raw(`
    ...
    ADD CONSTRAINT gbp_work_items_content_type_check
    CHECK (content_type IN ('review_reply', 'local_post'))
  `);
  await knex.raw(`
    ALTER TABLE ${WORK_ITEMS_TABLE}
    ADD CONSTRAINT gbp_work_items_status_check
    CHECK (status IN ('draft', 'awaiting_approval', 'approved', 'deploying', 'published', 'rejected'))
  `);
  await knex.raw(`
    CREATE UNIQUE INDEX gbp_work_items_active_review_reply_unique
    ON ${WORK_ITEMS_TABLE}(source_review_id)
  ...
  await knex.raw(`
    ...
    ADD CONSTRAINT gbp_deployment_attempts_status_check
