...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.schema.withSchema(SCHEMA).createTable(TARGETS_TABLE, (table) => {
    ...
    table
      ...
      .notNullable()
      .references("id")
      .inTable(tableName(RUNS_TABLE))
      .onDelete("CASCADE");
    table.text("target_type").notNullable();
    table
      .uuid("page_id")
      .references("id")
      .inTable(tableName("pages"))
      .onDelete("SET NULL");
    table
      .integer("location_id")
      .references("id")
  ...
  await knex.schema.withSchema(SCHEMA).createTable(RESULTS_TABLE, (table) => {
    ...
    table
      .uuid("target_id")
      .references("id")
      .inTable(tableName(TARGETS_TABLE))
      .onDelete("CASCADE");
  ...
  await knex.schema.withSchema(SCHEMA).createTable(EVIDENCE_TABLE, (table) => {
    ...
    table
      ...
      .notNullable()
      .references("id")
      .inTable(tableName(RESULTS_TABLE))
      .onDelete("CASCADE");
  ...
  await knex.raw(`
    ALTER TABLE ${tableName(RUNS_TABLE)}
    ADD CONSTRAINT ai_seo_runs_scope_check
    CHECK (scope IN ('url_only', 'organization', 'sitewide', 'location'))
  `);
  await knex.raw(`
    ALTER TABLE ${tableName(RUNS_TABLE)}
    ADD CONSTRAINT ai_seo_runs_status_check
    CHECK (status IN ('queued', 'running', 'completed', 'failed'))
  `);
  await knex.raw(`
    ALTER TABLE ${tableName(RUNS_TABLE)}
    ADD CONSTRAINT ai_seo_runs_confidence_check
    CHECK (confidence IS NULL OR confidence IN ('low', 'medium', 'high'))
  `);
  await knex.raw(`
    ALTER TABLE ${tableName(TARGETS_TABLE)}
    ADD CONSTRAINT ai_seo_targets_type_check
    CHECK (target_type IN ('page', 'location', 'site'))
  `);
  await knex.raw(`
    ALTER TABLE ${tableName(TARGETS_TABLE)}
    ADD CONSTRAINT ai_seo_targets_confidence_check
    CHECK (confidence IS NULL OR confidence IN ('low', 'medium', 'high'))
  `);
  await knex.raw(`
    ALTER TABLE ${tableName(RESULTS_TABLE)}
    ADD CONSTRAINT ai_seo_results_status_check
    CHECK (status IN ('pass', 'partial', 'fail', 'unavailable', 'not_applicable'))
  `);
  await knex.raw(`
    ALTER TABLE ${tableName(RESULTS_TABLE)}
    ADD CONSTRAINT ai_seo_results_method_check
    CHECK (method IN ('deterministic', 'llm_assisted', 'integration'))
  `);
  await knex.raw(`
    ALTER TABLE ${tableName(RESULTS_TABLE)}
    ADD CONSTRAINT ai_seo_results_scope_check
    CHECK (data_scope IN ('url', 'organization', 'location', 'external'))
  `);
  await knex.raw(`
    ALTER TABLE ${tableName(EXTERNAL_SOURCES_TABLE)}
    ADD CONSTRAINT ai_seo_sources_state_check
    CHECK (entity_match_state IN ('consistent', 'conflicting', 'missing_on_site', 'external_candidate', 'ambiguous_entity', 'unavailable'))
  `);
}
