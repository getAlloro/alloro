...
import { calculateProfileStrength } from "../../controllers/practice-ranking/feature-utils/util.competitor-profile-strength";
import {
  ReviewsMetrics,
  GbpMetrics,
  RankingMetrics,
...
const EMPTY_CHOOSABLE: ChoosableMetrics = {
...
/**
 * Choosable-stage READ: how the practice's PUBLIC review/rating profile stacks
 * against its curated `location_competitors` set. Best-effort, never throws.
 *
 * Practice side is echoed from the already-computed `reviews` section (one
 * source of truth per number — no second fetch). Competitor side comes from
 * `location_competitors` rows (captured at discovery/refresh, so honestly
 * qualified by `as_of`). All fields null/false when no competitor set exists.
 *
 * Owns the READ only. The review-ASK action is Chapter 6 (Memorable).
 */
export async function buildChoosableMetrics(
  ...
  try {
    ...
    const practiceRating = reviews.current_rating;

    // --- Competitor review counts (skip null) ----------------------
    const reviewCounts = competitors
      .map((c) => c.review_count)
      .filter((n): n is number => typeof n === "number" && Number.isFinite(n));
    const competitorMedianReviews = median(reviewCounts);

    // Strongest competitor = most reviews in the set.
    let strongest: { name: string; count: number } | null = null;
    for (const c of competitors) {
      if (typeof c.review_count === "number" && Number.isFinite(c.review_count)) {
        if (strongest === null || c.review_count > strongest.count) {
          strongest = { name: c.name, count: c.review_count };
        }
      }
    }

    const competitorsAheadOnReviews =
      practiceReviewCount !== null
        ? reviewCounts.filter((n) => n > practiceReviewCount).length
        : null;

    const reviewCountGapToMedian =
      competitorMedianReviews !== null && practiceReviewCount !== null
        ? Number((competitorMedianReviews - practiceReviewCount).toFixed(2))
        : null;

    const practiceLeadsOnReviews =
      competitorMedianReviews !== null && practiceReviewCount !== null
        ? practiceReviewCount >= competitorMedianReviews
        : null;

    // --- Freshness: oldest competitor discovery_checked_at ---------
    let asOf: string | null = null;
    for (const c of competitors) {
      if (c.discovery_checked_at) {
        const d =
          c.discovery_checked_at instanceof Date
            ? c.discovery_checked_at
            : new Date(c.discovery_checked_at as unknown as string);
        if (!Number.isNaN(d.getTime())) {
          if (asOf === null || d.getTime() < new Date(asOf).getTime()) {
            asOf = d.toISOString();
          }
        }
      }
    ...
    // --- FIX B: other Choosable dimensions (reuse the strength fn) --
    // Competitor profile strength recomputed from row fields via the canonical
    // pure function (genuine reuse, consistent scale). Median over non-null.
    const competitorStrengths = competitors
      ...
      .filter((n): n is number => typeof n === "number" && Number.isFinite(n));
    const competitorMedianProfileStrength = median(competitorStrengths);

    // HONESTY GATE (FIX B): the practice's OWN completeness factors
    // (website/phone/photo/category) are NOT reliably available in the current
    // dashboard GBP fetch. Because calculateProfileStrength coerces each
    // missing factor to `false` (scored identically to a confirmed absence), a
    // score built here would be a silently-deflated LOWER BOUND, not a
    // like-for-like comparison. Per the honesty gate we therefore leave
    // practice_profile_strength null (Value #6: never assert absence from a
    // null). Reviews/rating are the load-bearing, fully-grounded signals.
    const practiceProfileStrength: number | null = null;

    // Weakest Choosable factor — ONLY from factors we actually measured:
    // reviews (vs competitor median) and rating (vs competitor median rating).
    // NEVER "photo"/"website": the practice's own presence is not confirmed
    // here, so naming it would assert absence from a null.
    const ratings = competitors
      .map((c) => c.rating)
      .filter((n): n is number => typeof n === "number" && Number.isFinite(n));
    const competitorMedianRating = median(ratings);

    let weakestChoosableFactor: ChoosableWeakestFactor | null = null;
    const reviewGapFrac =
      competitorMedianReviews !== null &&
      competitorMedianReviews > 0 &&
      practiceReviewCount !== null &&
    ...
    const ratingGapFrac =
      ...
      competitorMedianRating > practiceRating
        ...
        : 0;
    if (reviewGapFrac > 0 || ratingGapFrac > 0) {
      weakestChoosableFactor =
        reviewGapFrac >= ratingGapFrac ? "reviews" : "rating";
    }
    ...
    return {
      ...
      strongest_competitor_name: strongest ? strongest.name : null,
      strongest_competitor_review_count: strongest ? strongest.count : null,
      competitors_ahead_on_reviews: competitorsAheadOnReviews,
      review_count_gap_to_median: reviewCountGapToMedian,
      practice_leads_on_reviews: practiceLeadsOnReviews,
      as_of: asOf,
      practice_profile_strength: practiceProfileStrength,
      competitor_median_profile_strength: competitorMedianProfileStrength,
      weakest_choosable_factor: weakestChoosableFactor,
    };
  } catch (err: any) {
    logger.warn(
