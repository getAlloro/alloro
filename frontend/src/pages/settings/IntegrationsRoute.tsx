      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            <div className="font-display text-base font-medium tracking-tight text-alloro-navy">
...
export const IntegrationsRoute: React.FC = () => {
  const { userProfile, hasProperties, hasGoogleConnection, refreshUserProperties } = useAuth();
  ...
  if (isLoading) {
    // Single-column skeleton mirroring the real stack (banner → cards → strip).
    return (
      <div className="space-y-5">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="animate-pulse rounded-2xl border border-black/5 bg-white p-5 shadow-premium"
          >
  ...
  // Single-column, priority-ordered stack: action banners → Google
  // integrations → Locations (the main object) → compact practice identity.
  // The old 5/7 two-column split cramped both halves inside the 960 shell.
  return (
    <div className="space-y-5">
      <section
      ...
      >
        ...
        {!hasGoogleConnection && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            className="bg-gradient-to-r from-orange-50 to-amber-50 border-2 border-alloro-orange/20 rounded-2xl p-5"
          >
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                <h3 className="font-display text-lg font-medium tracking-tight text-alloro-navy">
      ...
      {!isWizardActive && (
        <button
          ...
          className="w-full text-left px-4 py-2 rounded-xl border border-dashed border-slate-200 text-xs font-medium text-slate-400 hover:text-alloro-orange hover:border-alloro-orange/30 transition-all"
        >
          Restart Product Tour
        </button>
      )}
