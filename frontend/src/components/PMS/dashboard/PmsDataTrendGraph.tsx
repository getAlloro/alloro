...
export function PmsDataTrendGraph({
  ...
  const [hoveredMonth, setHoveredMonth] = useState<PmsDataTrendMonth | null>(
    null,
  );
  if (months.length === 0) return null;

  const productionMax = getMaxNullableValue(
    months.map((month) => month.productionTotal),
  );
  const referralMax = getMaxNullableValue(
    months.map((month) => month.totalReferrals),
  );
  ...
  const hoveredIndex = hoveredMonth
    ...
    : -1;
  const hoveredX =
    hoveredIndex >= 0 ? getDataTrendGraphX(hoveredIndex, months.length) : null;
  ...
  return (
    <div className="rounded-2xl border border-line-soft bg-white p-4 shadow-sm">
      ...
      <svg
        ...
        role="img"
        aria-label={`${copy.moneyLabel} and ${copy.countPlural} trend by month`}
      ...
      >
        ...
        {buildDataTrendGraphSegments(months, "totalReferrals", referralMax).map(
          (points) => (
            <polyline
      ...
      <div className="grid grid-cols-12 gap-1 text-center text-[9px] font-black uppercase tracking-[0.08em] text-alloro-navy/35">
        {months.map((month) => (
          <span
            key={month.month}
            className={month.isLatest ? "text-alloro-orange" : ""}
          >
            {month.label.split(" ")[0]}
          </span>
        ))}
...
function renderMonthPoints({
  ...
  const label = `${month.label}. ${formatDataTrendProduction(month.productionTotal, copy.moneyLower)}, ${formatDataTrendReferrals(month.totalReferrals, copy.countPlural)}. ${action}.`;
  const point = (y: number, className: string, key: string) => (
    <g
      key={key}
      role="button"
      tabIndex={0}
      ...
      onKeyDown={(event) =>
        handlePointKeyDown(event, month.month, onSelectMonth)
      }
    >
      <title>{label}</title>
  ...
  if (month.status !== "active") {
    const color =
      month.status === "ready"
        ? "stroke-alloro-orange"
        : "stroke-alloro-navy/35";
    return point(
      PMS_DATA_TREND_GRAPH.height - PMS_DATA_TREND_GRAPH.padY,
      color,
      month.month,
    );
  }
  ...
  return [
    point(
      getDataTrendGraphY(month.productionTotal, productionMax),
      "fill-white stroke-alloro-orange",
      `${month.month}-production`,
    ),
    point(
      getDataTrendGraphY(month.totalReferrals, referralMax),
      "fill-white stroke-emerald-700",
      `${month.month}-referrals`,
    ),
  ];
}
