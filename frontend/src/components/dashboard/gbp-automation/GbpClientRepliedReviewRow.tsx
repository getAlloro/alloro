...
export type GbpClientRepliedReviewRowProps = {
  review: GbpReview;
  isExpanded: boolean;
  isBusy: boolean;
  onToggle: () => void;
...
export function GbpClientRepliedReviewRow({
  review,
  isExpanded,
  isBusy,
  onToggle,
    ...
    <div className="rounded-[10px] border border-slate-200 bg-slate-50 p-3" onClick={onToggle}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          {!isExpanded && (
            <p className="mt-2 line-clamp-2 rounded-[9px] bg-white p-2 text-xs font-semibold leading-5 text-slate-600">
        ...
        <button
          ...
          className="inline-flex shrink-0 items-center justify-center gap-1.5 rounded-[9px] bg-alloro-navy px-3 py-2 text-[10px] font-black uppercase tracking-widest text-white transition-colors hover:bg-alloro-navy/90 disabled:cursor-not-allowed disabled:opacity-50"
        >
          <PencilLine size={12} />
          Manage
        </button>
      ...
      {isExpanded && (
        <div
          className="mt-3 rounded-[9px] border border-gray-200 bg-white p-3"
        ...
        >
          <textarea
            ...
            className="min-h-[92px] w-full resize-y rounded-[9px] border border-gray-200 bg-gray-50 p-3 text-sm font-medium leading-6 text-gray-700 outline-none transition focus:border-alloro-orange disabled:cursor-not-allowed disabled:opacity-60"
          />
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-[9px] border border-red-200 px-3 py-2 text-xs font-bold text-red-600 transition-colors hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50"
            >
              <Trash2 size={13} />
