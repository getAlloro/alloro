...
| I3 | **Set-integrity** — competitor lists **validate against org specialty at write time** and **alarm on mismatch** | **FROM-SCRATCH** | practice-ranking write path | Dominion-absent (H3) is the symptom of no set-integrity check: nothing catches a missing defining rival or an off-specialty entry. |
