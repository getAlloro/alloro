/**
 * Alloro OS → Admin Port — Knex migration (reference copy) · Rev 2
 * Plan: plans/07042026-alloro-os-admin-port
 *
 * Execution home: src/database/migrations/20260704000000_create_os_knowledge_base_tables.ts
 * (TypeScript, same content adapted to `import type { Knex } from "knex"`).
 *
 * Rev 2 (2026-07-04): all tables live in a DEDICATED SCHEMA `os` (owner ask),
 * mirroring the existing `minds` schema precedent (20260227000002). Table names
 * inside the schema are unprefixed. Models reference dot-qualified names —
 * `db("os.documents")` — exactly like MindBrainChunkModel's
 * `conn("minds.mind_brain_chunks")`. knex_migrations bookkeeping stays in public.
 *
...
exports.down = async function down(knex) {
  ...
  const drop = (name) => knex.schema.withSchema("os").dropTableIfExists(name);
  await drop("activity");
  await drop("comments");
