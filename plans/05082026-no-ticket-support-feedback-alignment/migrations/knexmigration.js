...
exports.up = async function up(knex) {
  ...
  await knex.schema.createTable(ATTACHMENTS_TABLE, (table) => {
    ...
    table.uuid("ticket_id").notNullable().references("id").inTable(TICKETS_TABLE).onDelete("CASCADE");
    table.integer("uploaded_by_user_id").references("id").inTable("users").onDelete("SET NULL");
  ...
  await knex.raw(`ALTER TABLE ${ATTACHMENTS_TABLE} ADD CONSTRAINT ${ATTACHMENTS_TABLE}_uploader_role_check CHECK (uploader_role IN ('client', 'admin', 'system'))`);
  await knex.raw(`ALTER TABLE ${ATTACHMENTS_TABLE} ADD CONSTRAINT ${ATTACHMENTS_TABLE}_visibility_check CHECK (visibility IN ('client_visible', 'internal'))`);
  await knex.raw(`CREATE INDEX idx_support_ticket_attachments_ticket ON ${ATTACHMENTS_TABLE}(ticket_id, created_at DESC)`);
  await knex.raw(`CREATE INDEX idx_support_ticket_attachments_ticket_visibility ON ${ATTACHMENTS_TABLE}(ticket_id, visibility, created_at DESC)`);
};
