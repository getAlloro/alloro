...
**Change:** Replaced the Rev 7 hover *chip* — Dave wanted the headline **components** to update on hover, not a floating tooltip. Now hovering a sparkline point updates that chart's own headline number **and period verbiage** (hover Feb → "N · Feb 2026"): the hero funnel Visitors, the Traffic card (visitors + sessions/page-views + date), the Leads card (leads + month), and both detail-modal chart titles. `TrendSparkline` reverted to a plain `onActiveIndexChange` callback (chip removed); each parent owns its hover index. `websiteMetrics.visitorSeries` enriched with per-day sessions/pageviews; lead-series month labels humanised ("2026-02" → "Feb 2026"). Deltas hide while a point is hovered (a single day/month has no MoM delta).
