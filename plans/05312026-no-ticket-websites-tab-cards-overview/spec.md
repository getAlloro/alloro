...
**Root cause:** sessions & pageviews are additive across days, so summing daily harvests = the month. **Unique visitors are NOT additive** — a person visiting on N days = N daily-uniques but 1 monthly-unique. We sum daily `users` ([websiteMetrics.ts] monthVisitors + monthly buckets, and `summarizeDaily` totals.users in [service.rybbit-performance.ts]); Rybbit dedupes per period. So our **"Visitors" runs ~10-13% high**, which also **deflates the conversion rate** (denominator too big). Timezone (`America/New_York` hardcoded in [rybbitHarvestAdapter.ts:40] + [service.rybbit-history.ts:11]) is a near-non-issue: NY vs UTC moved sessions ~0.3%. Ruled out: missing days (0 gaps in every account), and a `sessions`/`visits` field mix (payload has no `visits`). **Fix options pending Dave's decision** (read-time period-unique query vs harvest monthly-uniques vs switch headline to sessions vs relabel "Visitors"→"Visits").
