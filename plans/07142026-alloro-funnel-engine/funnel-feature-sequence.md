...
- **M0 — Submission source-capture** (see Measurement rail below). Prove first: without attribution every later lever is invisible. Bounded engineering (migration + capture referrer/source on submit + retire the orphaned `firstPatientAttribution.ts`), independent PR. **Highest leverage per hour on the board.**
