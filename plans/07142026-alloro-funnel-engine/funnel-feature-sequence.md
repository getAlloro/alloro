...
7. **B1 — Instrument the hosted/preview site** (near-free; makes "considered" measurable — prerequisite for judging B2/B3). **← IN PROGRESS 2026-07-15** — spec `plans/07152026-instrument-site/spec.html`; re-scoped after serve-path trace (inject point lives in a separate `website-renderer` repo — B1 = gated on-demand Rybbit provisioning for preview projects; pageview/session/bounce turns on). Follow-on **B1-R** (form-view/contact-click custom events + renderer-snippet PII audit) needs `website-renderer` access.
