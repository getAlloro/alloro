...
> ---
>
> ## ⛔ MERGED ≠ WORKING — read this before you trust any "✅ DONE / PR'd / COMPLETE" mark below.
> **This file marks a feature "DONE/PR'd" when a PR exists or merged. That is NOT the same as live-and-moving-a-number.** The A1–A6 get-found PRs (#164–168, #183–187, #193) **did merge to `dev/dave` and are largely on `main`** — but merging code is not wiring it, enabling it, or proving it. **For the TRUE customer-facing state, `docs/capability-ledger.md` is the source, not this file.** As of 2026-07-21 the ledger's honest read of get-found is:
> - **The ONLY get-found lever that is LIVE-and-wired is GF3 — "fill the website URL" — and it is a ≈no-op** (profiles already have websites). This is the "better dashboard that updates a website URL" reality.
> - **The strong levers are merged-but-not-working:** GF2 primary category (strongest lever) = **BUILT–NOT-WIRED** (no running caller), GF5 A6 write-back = **GATED-OFF** (default false), GF1 impressions gauge = **BLOCKED** (reads 0, zero-Maps), GF4 phone/hours = **FROM-SCRATCH**, GF7 GSC→content = **SHIPPED–VERIFY**, GF8 AEO schema = **PARTLY BUILT**.
> - So the honest get-found status is **NOT "COMPLETE."** It is: *code merged, ~one no-op lever live, every lever with leverage still off/unwired/blocked.* The real remaining work is Layer 1 (wire + enable) + Layer 0 (fix the zero-Maps instrument) + proving lift — not "ship B1 next."
> *(Coherence fix C2, 2026-07-21: every "DONE/COMPLETE/PR'd" below means "PR merged," never "working." Where they conflict, the ledger wins.)*
>
> ---
>
