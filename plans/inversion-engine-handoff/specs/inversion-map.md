...
3. **Findable (Stage 2).** What the owner ought to see about showing up when patients search. All public data. STATUS: spec complete, not yet built as code.
4. **Choosable (Stage 3).** The stage where the most revenue leaks (per the lattice), all public. STATUS: spec complete, not yet built as code.
5. **Bookable (Stage 4).** The most fixable stage: phone, forms, online booking. STATUS: spec complete, not yet built as code.
6. **Memorable (Stage 5).** Reviews, referrals, follow-up. The compounding stage. STATUS: spec complete, not yet built as code.
...
**⚠️ Coherence pass (2026-07-07, AFTER the fix-pass above):** a system-wide audit found deeper gaps the per-chapter fix-pass missed. These, not the fix-pass residuals, are the real blockers before building past Ch2, and they were resolved in the spec-resolution pass (see the ✅ RESOLVED items below):
