...
- **Stay in lane.** Presence-side by default. AI drafts; a human approves before anything publishes. **Outbound to a human is OWNER-APPROVED ONLY (Option B, staked by Corey 2026-07-08, this UPDATES the prior "never outbound" rule):** Alloro CAN send outbound (review requests, lead replies) when the owner / responsible party approves each send (drafted, owner confirms, then sent). NEVER auto-send, NEVER unsolicited (no marketing/nurture blasts). The governing intent for edge cases: keep AI-error at or below human-error so nothing wrong reaches the customer, owner-approval IS that gate. (Forward hook: revisit auto-send per-action only when AI is proven at or below human error rate, evidence-gated, not dogma.) So Ch5a builds presence-side PLUS an owner-approved outbound lead-reply action, not just the narrow transactional auto-ack.
...
- **Spec-level gaps the pre-flight found, FIX OR FLAG before those chapters build:**
  ...
  - **Coherence-pass spec residuals (build-time + Corey-stakes):** (a) the "pushed digest" Ch2/Ch3/Ch7 call the owner's REAL surface is asserted but NEVER built, either scope those fixes honestly to the logged-in surface, or build the digest; don't ship the owner-glance to a surface the owner never visits. (b) Ch6's velocity card ends in a review-ASK the owner performs (unbuilt rail); Corey's Value-#2 stake, is a bounded ask a legitimate owner-capable action, or homework to make pure read-only? (c) the cumulative "Alloro did X" voice across chapters can tip toward Alloro-as-hero, keep the OWNER the hero ("your reviews are answered, approve and it's done in your name"). (d) Ch7 hard-codes `form-submission` as "not yet live", key it off a runtime capability flag once Ch5a ships. (e) reviews stage-label: the unified TYPE carries an explicit `stage` field, the eyebrow reads it (with `DOMAIN_TO_STAGE` as FALLBACK, not removed), reviews are staked **Choosable** owner-facing (Corey 2026-07-07), and the verdict names the stage from the stat→stage map. Was Gap 7; see inversion-07 FIX 3.

---

## 5. RECOMMENDED SEQUENCE (Dave decides; this is a read, not an order)

> **CURRENT STATE 2026-07-08 (the steps below are the original 07-07 read; this is where it actually stands):** Step 1 is DONE, Slice 1 is MERGED (PR #145). Step 2 is DONE, the focus-dashboard rank fix is already on `origin/dev/dave` (commit `aac57428`; `sectionBuilders.ts` reads `search_position`). PR #147 still shows open but its work is on the branch. Step 3, the foundation (Ch2 + Ch7), is OPEN as PR #148. So your first clean NEW build is a stage chapter: **Ch3 (Findable)** or **Ch4 (Choosable)**, both are complete specs, both feed the foundation selector, and neither carries an open Corey-stake (Ch5a + Ch6 do).
