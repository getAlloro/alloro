...
- `stage`: the authoring journey stage (`findable` | `choosable` | `bookable` | `memorable`). Set by the generating chapter, NOT derived from `domain` by a static map (that mislabeled Memorable review-actions as "Choosable"). Ch7's eyebrow reads THIS.
