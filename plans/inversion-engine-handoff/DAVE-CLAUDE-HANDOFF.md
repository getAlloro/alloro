...
- **Routing note:** work normally reaches Dave via the Visionary → Integrator → Jo's board → Dave chain. This engine handoff routes DIRECTLY to Dave, it's a one-time whole-engine transfer AND Jo is on leave, so Corey holds Jo's sign-off gate in the meantime. The approver on anything below is Corey (async), not Jo's board.
